...
@section('body')
    ...
    <div class="container -mt-[74px] top-0 bg-fixed mx-auto bg-cover bg-center p-6 h-full"
        style="background-image: url({{ asset('storage/aurora.jpg') }})">
        ...
        <form method="POST" action="{{ route('booking.store') }}"
            ...
            <div class="grid gap-6 mb-6 lg:grid-cols-2">
                <div>
                    <label for="nama" class="block my-2 text-sm font-medium text-white">Nama
                        Gedung</label>
                    <input type="text" id="gedung_nama" value="{{ $bookings->gedung->nama }}"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        readonly>
                    <input name="gedung_id" value="{{ $bookings->gedung->id }}" type="hidden"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                </div>
                <div>
                    <label for="no_telp" class="block my-2 text-sm font-medium text-white">No
                        Handphone</label>
                    <input type="tel" id="no_telp" name="no_telp" value="{{ $bookings->no_telp }}"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    @foreach ($bookings->fasilitas as $fass)
                            ...
                            <div class="block">
                                <div class="inline-flex">
                                    <input class="fasilitas-checkbox flex items-center space-x-5 mx-2"
                                        ...
                                        value="{{ $fass->id }}" @if ($fass->pivot->jumlah_unit > 0) checked @endif>
                                    <span class="text-white">{{ $fass->fasilitas }} : {{ $fass->harga }}/unit</span>
                                </div>
                                <div>
                                    <input style="display: block"
                                        ...
                                        min="0" value="{{ $fass->pivot->jumlah_unit }}"
                                        @if ($fass->pivot->jumlah_unit > 0)  @endif>
                                </div>
                            </div>
                        @endif
                    @endforeach
            ...
            <div>
                ...
                <input type="number" id="harga_total" name="harga_total"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder="Total harga" value="{{ $bookings->gedung->harga }}" readonly>
            </div>
            <div class="mt-8">
                <button type="submit"
    ...
    <script>
        ...
        function hitungTotalHarga() {
            let totalHarga = {{ $bookings->gedung->harga }}; // Nilai gedungs->harga
